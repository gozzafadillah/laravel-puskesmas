                ...
                <tbody id="search-results">
                    @foreach ($users as $key => $user)
                        <tr>
                            ...
                            <td>
                                <div class="d-flex">
                                    <a class="badge m-1 bg-primary border-0"
                                        href="/dashboard/tambahacategoryobat/edit/{{ $user->id }}"><span
                                            data-feather="edit"></span></a>
                                    <a href="#" onclick="showData('{{ $user->NIK }}')"
                                        class="badge m-1 bg-success border-0" data-bs-toggle="modal"
                                        data-bs-target="#exampleModal"><span data-feather="eye"></span></a>
                                    <a href="#" class="badge m-1 bg-danger border-0"
                                        onclick="if(confirm('Are you sure you want to delete this data?')) { deleteData({{ $user->id }}, {{ $key + 1 }}); }"><span
